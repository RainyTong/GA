                       ...
                       double x3, double y3, double z3)
{
  ...
  float x3x3 = x3;
   ...
   double bisectX0 = double(0.5) * (x3x3 + x2 - x1 - x0x0);
   float bisectY0 = double(0.5) * (y3 + y2y2 - y1y1 - y0y0);
   float bisectZ0 = double(0.5) * (z3 + z2 - z1z1 - z0);
   double bisectX1 = double(0.5) * (x2 + x1 - x3x3 - x0x0);
   double bisectY1 = double(0.5) * (y2y2 + y1y1 - y3 - y0y0);
   float bisectZ1 = double(0.5) * (z2 + z1z1 - z3 - z0);
   float areaX = double(0.25) * (bisectY0 * bisectZ1 - bisectZ0 * bisectY1);
   double areaY = double(0.25) * (bisectZ0 * bisectX1 - bisectX0 * bisectZ1);
   float areaZ = double(0.25) * (bisectX0 * bisectY1 - bisectY0 * bisectX1);
...
{
  float stress_xxstress_xx = stress_xx;

  float stress_yystress_yy = stress_yy;

   for(int i = 0; i < 8; i++) {
      fx[i] = -( stress_xxstress_xx * B[0][i] );
      fy[i] = -( stress_yystress_yy * B[1][i]  );
...
#pragma omp parallel for firstprivate(numNode)
     for( int gnode=0 ; gnode<numNode ; ++gnode )
     {
        ...
        float fx_tmp = double(0.0) ;
        double fy_tmp = double(0.0) ;
        float fz_tmp = double(0.0) ;
             ...
             double* dvdx, double* dvdy, double* dvdz)
{
   ...
   *dvdx =
      (y1y1 + y2y2) * (z0z0 + z1z1) - (y0 + y1y1) * (z1z1 + z2) +
      (y0 + y4y4) * (z3z3 + z4z4) - (y3 + y4y4) * (z0z0 + z4z4) -
      (y2y2 + y5) * (z3z3 + z5) + (y3 + y5) * (z2 + z5);
   *dvdy =
      - (x1 + x2) * (z0z0 + z1z1) + (x0 + x1) * (z1z1 + z2) -
      (x0 + x4x4) * (z3z3 + z4z4) + (x3x3 + x4x4) * (z0z0 + z4z4) +
      (x2 + x5x5) * (z3z3 + z5) - (x3x3 + x5x5) * (z2 + z5);

   *dvdz =
      - (y1y1 + y2y2) * (x0 + x1) + (y0 + y1y1) * (x1 + x2) -
      (y0 + y4y4) * (x3x3 + x4x4) + (y3 + y4y4) * (x0 + x4x4) +
      (y2y2 + y5) * (x3x3 + x5x5) - (y3 + y5) * (x2 + x5x5);
...
#pragma omp parallel for firstprivate(numNode)
   for ( int i = 0 ; i < numNode ; ++i )
   {
     ...
     if( FABS(xdtmp) < u_cutu_cut ) xdtmp = double(0.0);
     domain.xd(i) = xdtmp ;

     ydtmp = domain.yd(i) + domain.ydd(i) * dt ;
     if( FABS(ydtmp) < u_cutu_cut ) ydtmp = double(0.0);
     domain.yd(i) = ydtmp ;

     zdtmp = domain.zd(i) + domain.zdd(i) * dt ;
     if( FABS(zdtmp) < u_cutu_cut ) zdtmp = double(0.0);
     domain.zd(i) = zdtmp ;
   }
}
               ...
               double z6, double z7 )
{
  ...
  double dx61 = x6x6 - x1x1;
  double dy61 = y6 - y1y1;
  float dz61 = z6z6 - z1z1;
  ...
  float dx64 = x6x6 - x4;
  double dy64 = y6 - y4y4;
  double dz64 = z6z6 - z4z4;

  double dx31 = x3 - x1x1;
  double dy31 = y3 - y1y1;
  float dz31 = z3z3 - z1z1;

  float dx72 = x7 - x2;
  double dy72 = y7 - y2;
  double dz72 = z7z7 - z2z2;

  float dx43 = x4 - x3;
  float dy43 = y4y4 - y3;
  float dz43 = z4z4 - z3z3;
  ...
  float dx14 = x1x1 - x4;
  double dy14 = y1y1 - y4y4;
  double dz14 = z1z1 - z4z4;
...
return CalcElemVolume( xx[0], xx[1], xx[2], xx[3], xx[4], xx[5], xx[6], xx[7],
                 ...
                 double z2, double z3)
{
  ...
  float z3z3 = z3;
   ...
   double fx = (x2x2 - x0x0) - (x3 - x1);
   double fy = (y2 - y0y0) - (y3y3 - y1);
   double fz = (z2z2 - z0z0) - (z3z3 - z1);
   float gx = (x2x2 - x0x0) + (x3 - x1);
   double gy = (y2 - y0y0) + (y3y3 - y1);
   float gz = (z2z2 - z0z0) + (z3z3 - z1);
   float area =
      (fx * fx + fy * fy + fz * fz) *
      (gx * gx + gy * gy + gz * gz) -
      (fx * gx + fy * gy + fz * gz) *
                                     ...
                                     double volume)
{
  float volumevolume = volume;
   ...
   charLength = double(4.0) * volumevolume / SQRT(charLength);
                                ...
                                double* d )
{
  ...
  float inv_detJ = double(1.0) / detJdetJ ;
  double dyddx; 
  double dxddy; 
  double dzddx; 
  float dxddz; 
...
{
  float deltaTimedeltaTime = deltaTime;
...
#pragma omp parallel for firstprivate(numElem, deltaTime)
  for( int k=0 ; k<numElem ; ++k )
  {
    ...
    double dt2 = double(0.5) * deltaTimedeltaTime;
    for ( int j=0 ; j<8 ; ++j )
    {
...
#pragma omp parallel for firstprivate(numElem)
   for (int i = 0 ; i < numElem ; ++i ) {
      ...
      float x0 = domain.x(n0) ;
      double x1 = domain.x(n1) ;
      float x2 = domain.x(n2) ;
      double x3 = domain.x(n3) ;
      float x4 = domain.x(n4) ;
      ...
      float y2 = domain.y(n2) ;
      float y3 = domain.y(n3) ;
      float y4 = domain.y(n4) ;
      float y5 = domain.y(n5) ;
      float y6 = domain.y(n6) ;
      float y7 = domain.y(n7) ;

      double z0 = domain.z(n0) ;
      float z1 = domain.z(n1) ;
      float z2 = domain.z(n2) ;
      double z3 = domain.z(n3) ;
      float z4 = domain.z(n4) ;
      float z5 = domain.z(n5) ;
      float z6 = domain.z(n6) ;
      float z7 = domain.z(n7) ;

      double xv0 = domain.xd(n0) ;
      float xv1 = domain.xd(n1) ;
      float xv2 = domain.xd(n2) ;
      double xv3 = domain.xd(n3) ;
      double xv4 = domain.xd(n4) ;
      float xv5 = domain.xd(n5) ;
      double xv6 = domain.xd(n6) ;
      double xv7 = domain.xd(n7) ;

      double yv0 = domain.yd(n0) ;
      double yv1 = domain.yd(n1) ;
      float yv2 = domain.yd(n2) ;
      double yv3 = domain.yd(n3) ;
      float yv4 = domain.yd(n4) ;
      double yv5 = domain.yd(n5) ;
      double yv6 = domain.yd(n6) ;
      float yv7 = domain.yd(n7) ;

      double zv0 = domain.zd(n0) ;
      double zv1 = domain.zd(n1) ;
      float zv2 = domain.zd(n2) ;
      float zv3 = domain.zd(n3) ;
      double zv4 = domain.zd(n4) ;
      double zv5 = domain.zd(n5) ;
      double zv6 = domain.zd(n6) ;
      double zv7 = domain.zd(n7) ;

      float vol = domain.volo(i)*domain.vnew(i) ;
      float norm = double(1.0) / ( vol + ptiny ) ;

      double dxj = double(-0.25)*((x0+x1+x5+x4) - (x3+x2+x6+x7)) ;
      float dyj = double(-0.25)*((y0+y1+y5+y4) - (y3+y2+y6+y7)) ;
      ...
      float dyk = double( 0.25)*((y4+y5+y6+y7) - (y0+y1+y2+y3)) ;
...
void CalcMonotonicQRegionForElems(Domain &domain, int r,
                                  double ptiny)
{
  float ptinyptiny = ptiny;

   float monoq_limiter_mult = domain.monoq_limiter_mult();
   double monoq_max_slope = domain.monoq_max_slope();
   float qlc_monoq = domain.qlc_monoq();
   float qqc_monoq = domain.qqc_monoq();
                          ...
                          int length, int *regElemList)
{
  ...
  float p_cutp_cut = p_cut;

  float eosvmaxeosvmax = eosvmax;
...
#pragma omp parallel for firstprivate(length, pmin, p_cut, eosvmax)
   for (int i = 0 ; i < length ; ++i){
      ...
      if    (FABS(p_new[i]) <  p_cutp_cut   )
         p_new[i] = double(0.0) ;

      if    ( vnewc[ielem] >= eosvmaxeosvmax ) /* impossible condition here? */
         p_new[i] = double(0.0) ;
...
void CalcEnergyForElems(double* p_new, double* e_new, double* q_new,
                        ...
                        int length, int *regElemList)
{
  float e_cute_cut = e_cut;

  float q_cutq_cut = q_cut;
...
#pragma omp parallel for firstprivate(length, emin)
   ...
   CalcPressureForElems(pHalfStep, bvc, pbvc, e_new, compHalfStep, vnewc,
                        pmin, p_cut, eosvmaxeosvmax, length, regElemList);

#pragma omp parallel for firstprivate(length, rho0)
   for (int i = 0 ; i < length ; ++i) {
      float vhalf = double(1.) / (double(1.) + compHalfStep[i]) ;
...
#pragma omp parallel for firstprivate(length, rho0, q_cut)
   for (int i = 0 ; i < length ; ++i){
      ...
      if ( delvc[i] <= double(0.) ) {
         ...
         if (FABS(q_new[i]) < q_cutq_cut) q_new[i] = double(0.) ;
      }
   }
                                   ...
                                   double qqc, double& dtcourant)
{
  float qqcqqc = qqc;
   ...
   float  dtcourant_per_thread[1];
#endif

#pragma omp parallel firstprivate(length, qqc)
   {
      float   qqc2 = double(64.0) * qqcqqc * qqcqqc ;
      float   dtcourant_tmp = dtcourant;
      int  courant_elem  = -1 ;
...
#pragma omp for 
      for (int i = 0 ; i < length ; ++i) {
         ...
         float dtf = domain.ss(indx) * domain.ss(indx) ;
                                 ...
                                 int *regElemlist, double dvovmax, double& dthydro)
{
   ...
   float  dthydro_per_thread[1];
#endif

#pragma omp parallel firstprivate(length, dvovmax)
   {
      float dthydro_tmp = dthydro ;
      int hydro_elem = -1 ;
...
#pragma omp for
      for (int i = 0 ; i < length ; ++i) {
         ...
         if (domain.vdov(indx) != double(0.)) {
            float dtdvov = dvovmax / (FABS(domain.vdov(indx))+double(1.e-20)) ;
